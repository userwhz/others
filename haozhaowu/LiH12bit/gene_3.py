...
def truncate_operator(op: SparsePauliOp, ratio: float) -> SparsePauliOp:
    """保留模值最大的前 ratio 比例的项"""
    if ratio >= 1.0:
        return op

    coeffs_abs = np.abs(op.coeffs)
    num_terms = len(op)
    keep_n = max(1, int(num_terms * ratio))

    indices = np.argsort(coeffs_abs)[-keep_n:]
    truncated_op = op[indices]
    return truncated_op.simplify()


# 修改后的 save_pauli_txt 函数：去掉了中间换行的逻辑
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate LiH STO-3G Hamiltonians (jw/parity/bk) and quantum states with truncation."
    )
    parser.add_argument(
        "--atom",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3333,
        help="Ratio of terms to keep (e.g., 0.33 for top 1/3 terms by magnitude).",
    )
    ...
    for mapper_name, mapper in get_mappers().items():
        ...
        original_len = len(qubit_op)
        qubit_op = truncate_operator(qubit_op, args.truncate_ratio)
        truncated_len = len(qubit_op)
        ...
        print(
            f"[{mapper_name}] qubits={qubit_op.num_qubits}, original_terms={original_len}, truncated_terms={truncated_len}, E0={e0:.12f}")
