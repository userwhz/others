"""
从 *_pauli.txt（与 gen_fermionic / h2o.py 相同格式：Pauli 串一行、系数一行）
生成稠密 *.npy。

优先用 Qiskit：见文件顶部 `from qiskit.quantum_info import SparsePauliOp` 与 `_HAS_QISKIT`，
`SparsePauliOp.to_matrix`（与 h2o.py 中 save_hamiltonian_dense_npy 一致、快）；
...
def main() -> None:
	...
	parser.add_argument(
		"--pauli-txt",
		type=str,
		default=os.path.join(base_dir, "hamiltonian_H2O_sto3g_14_pauli.txt"),
		help="输入 Pauli 文本路径",
	)
	parser.add_argument(
		"--out-npy",
