function [timeSpend] = main_CUTOGMV2(file, indice, T)
...
strS = strcat(str_set, num2str(indice));
strS = strcat(strS, file);
observ = load(strF);
...
%     end
    ...
    [pr, diagonal_var, exitflag] = OptDiagVar2(pr, upper_B);
end
if exitflag == 0
    [pr, diagonal_var, exitflag] = OptDiagVar2(pr, 100);
end
