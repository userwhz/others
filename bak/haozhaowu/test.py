...
def main():
    ...
    if iterm <= 4.3:
        # 加载pkl文件
        with open(f'H2_{iterm:.1f}_sto-3g.pkl', 'rb') as f:
            data = pickle.load(f)
        ...
        np.save(f'ground_state_H6_{iterm:.1f}_sto-3g.npy', ground_state_vector)
        np.save(f'ground_state_H6_{iterm:.1f}_sto-3g_density_matrix.npy', density_matrix)
        ...
        # 2. 保存成附件格式
        print("\n2. 保存为附件格式...")
        pauli_terms = save_hamiltonian_in_attachment_format(data['qubit_hamiltonian'], data['num_qubits'], f'H6_{iterm:.1f}_sto-3g_sg.txt')
        ...
        # 3. 可选：保存矩阵到文件
        np.save(f'H6_{iterm:.1f}_sto-3g.npy', hamiltonian_matrix)
