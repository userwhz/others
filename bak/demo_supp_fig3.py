...
if __name__ == "__main__":
    ...
    filtered_prov = Nrec >= threshold_prov
    filtered_prov_new = Nrec_OGM >= threshold_prov
    N_prov = np.logspace(np.log10(threshold_prov),x_max_log,100)
    ...
    for j,(mapping,map_dict) in enumerate(benchmark_dict["NH3"].items()):
        ...
        for i,method in enumerate(keys):
            ...
            if method =="OverlappedGrouping":
                ...
                Nvals = Nrec
                filt_p = filtered_prov
            provable = np.array(map_dict["provable"][method])
            popt,pcov = fit_params[mapping]["provable"].get(method,([0,0], None))
            if popt[-1] > 0.05:
                print(method,":",popt[-1],"--> N_chem.acc. (provably):", get_N_estimates(*popt)[1])
                temp = fit_func(N_prov,*popt)
                plt.plot(N_prov,temp,color=cmap(i),linestyle="dotted")

            plt.plot(Nvals,provable,label=label,color=cmap(i),linewidth=3)
        yticks = np.array( [10,provable[0]] )
        plt.ylim(bottom=10**y_min_log)
        if j==1:
            plt.legend(fontsize="large")
            plt.xlabel("Guaranteed accuracy (S32) [Ha] over no. meas. rounds $N$",fontsize="xx-large")
        plt.grid()
        plt.xlim(100,N_prov[-1])
        plt.xticks(fontsize="x-large")
        ypos = 55
        plt.text(3e5,ypos,mapping,fontsize="xx-large",horizontalalignment="center")
        if j==2:
    ...
    for j,mapping in enumerate(benchmark_dict["NH3"].keys()):
        ...
        for i,method in enumerate(methods):
            for k,molecule in enumerate(available_molecules):
                ...
                temp = fit_params[mapping]["provable"].get(method,None)
                if temp is not None:
        ...
        plt.yscale('log')
        if j==1:
            plt.xlabel('No. meas. rounds to provably reach chem. acc.', fontsize = "xx-large")
        plt.xticks([])
        plt.grid(axis="y",zorder=1)
        if j==2:
            plt.gca().yaxis.tick_right()
            plt.gca().yaxis.set_label_position("right")
        plt.ylim(1e12,top_threshold)
        ...
        if j==1:
            plt.text(2,0.45,'Fit exponent c', fontsize = "xx-large",horizontalalignment="center")
        plt.xticks(np.arange(len(available_molecules)),
                [mol.split("_")[0] for mol in available_molecules],fontsize="x-large")
        plt.grid(axis="y",zorder=1)
        plt.ylim(0,0.5)
        if j==2:
            plt.gca().yaxis.tick_right()
    ...
    plt.savefig("generated_figures/supp_fig3_demo.png",bbox_inches="tight")
    plt.show()
