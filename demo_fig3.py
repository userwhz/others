...
if __name__ == "__main__":
    ...
    for i,method in enumerate(keys):
        if method =="OverlappedGrouping":
            ...
            Nvals = Nrec
            filt_p = filtered
        emp,std = benchmark_dict["NH3"]["JW"]["empirical"][method]
        emp,std = np.array(emp),np.array(std)
        popt,pcov = fit_params["JW"]["empirical"].get(method,([0,0], None))
        if popt[-1] > 0.05:
        ...
        plt.errorbar(Nvals,emp,yerr=std/10,label=method,ecolor=cmap(i),linewidth=0,elinewidth=5)
    ...
    plt.savefig("generated_figures/fig3_demo.png",bbox_inches="tight")
    plt.show()
